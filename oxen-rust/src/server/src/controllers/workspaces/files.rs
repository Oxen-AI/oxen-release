...
pub async fn get(
    ...
    let file_node = repositories::tree::get_file_by_path(&workspace.base_repo, &workspace.commit, &path)?
    ...
    if img_resize.width.is_some() || img_resize.height.is_some() {
        ...
        let metadata = fs::metadata(&resized_path).await?;
    ...
    // Stream the file
    let (stream, file_size) = version_store
