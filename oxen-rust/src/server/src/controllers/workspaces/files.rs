...
pub async fn save_stream(
    ...
    log::debug!("workspace::files::save_stream Got workspace dir: {workspace_dir:?}");

    let full_dir = workspace_dir.join(filepath);

    log::debug!("workspace::files::save_stream Got full dir: {full_dir:?}");

    if let Some(parent) = full_dir.parent() {
        std::fs::create_dir_all(parent)?;
    }

    log::debug!("workspace::files::save_stream successfully created full dir: {full_dir:?}");
    ...
    log::debug!("workspace::files::save_stream is writing to file: {file:?}");
