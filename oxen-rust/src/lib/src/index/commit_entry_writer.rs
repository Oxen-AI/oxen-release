...
use crate::model::{
    Commit, CommitEntry, LocalRepository, StagedDirStats, StagedEntry, StagedEntryStatus,
};
use crate::util;
...
impl CommitEntryWriter {
    ...
    fn add_modified_to_db(&self, new_commit: &Commit, path: &Path) -> Result<(), OxenError> {
    ...
    fn add_path_to_db(&self, new_commit: &Commit, path: &Path) -> Result<(), OxenError> {
    ...
    pub fn add_commit_entry(&self, commit_id: &str, entry: &CommitEntry) -> Result<(), OxenError> {
    ...
    fn backup_file_to_versions_dir(
        ...
        let versions_entry_dir =
            CommitEntryWriter::versions_dir(&self.repository.path).join(&new_entry.id);
    ...
    fn add_staged_files(&self, commit: &Commit, added_files: &[PathBuf]) -> Result<(), OxenError> {
        // len kind of arbitrary right now...just nice to see progress on big sets of files
        if added_files.len() > 1000 {
    ...
    fn commit_staged_entry(&self, commit: &Commit, path: &Path, entry: &StagedEntry) {
        match entry.status {
            ...
            StagedEntryStatus::Modified => match self.add_modified_to_db(commit, path) {
                Ok(_) => {}
                Err(err) => {
            ...
            StagedEntryStatus::Added => match self.add_path_to_db(commit, path) {
                Ok(_) => {}
                Err(err) => {
