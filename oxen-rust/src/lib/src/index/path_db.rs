...
{
    let path = path.as_ref();
    log::debug!("path_db::get_entry({:?})", path);
    if let Some(path_str) = path.to_str() {
        let bytes = path_str.as_bytes();
        match db.get(bytes) {
            Ok(Some(value)) => {
                ...
                log::debug!("path_db::get_entry({:?}) GOT IT", path);
                Ok(Some(entry))
            }
            Ok(None) => {
                // did not get val
                log::debug!("path_db::get_entry({:?}) don't got it....", path);
                Ok(None)
            }
            Err(err) => {
                // error from the DB
                let err = format!("Err could not fetch value from db: {}", err);
    ...
    T: Serialize,
{
    ...
    log::debug!("add_to_db {:?} -> {:?}", path, entry_json);
