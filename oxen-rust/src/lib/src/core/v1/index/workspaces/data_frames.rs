...
use crate::core::v1::index::{self, workspaces};
...
pub fn index(workspace: &Workspace, path: &Path) -> Result<(), OxenError> {
    ...
    log::debug!(
        "core::v1::index::workspaces::data_frames::index({:?}) got version path: {:?}",
        entry.path,
        version_path
    );
    ...
    log::debug!(
        "core::v1::index::workspaces::data_frames::index({:?}) finished!",
        entry.path
    );
    ...
    fn test_restore_row() -> Result<(), OxenError> {
        ...
        test::run_training_data_repo_test_fully_committed(|repo| {
            ...
            let file_path = Path::new("annotations")
                ...
                .join("bounding_box.csv");
            let commit_entry =
                repositories::entries::get_commit_entry(&repo, &commit, &file_path)?.unwrap();
