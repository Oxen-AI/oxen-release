...
mod tests {
    ...
    async fn test_remote_mode_rm_dir_that_is_not_committed_should_throw_error() -> Result<(), OxenError> {
        test::run_empty_remote_repo_test(|_local_repo, remote_repo| async move {
            ...
            test::run_empty_dir_test_async(|dir| async move {
                ...
                let result = api::client::workspaces::files::rm_files(&remote_repo, &workspace_id, paths_to_remove).await;
    ...
    async fn test_remote_mode_rm_one_file_in_dir() -> Result<(), OxenError> {
        test::run_empty_remote_repo_test(|_local_repo, remote_repo| async move {
            ...
            test::run_empty_dir_test_async(|dir| async move {
                ...
                let file1 = images_dir.join("dog_1.jpg");
                let file2 = images_dir.join("dog_2.jpg");
                let file3 = images_dir.join("cat_1.jpg");
                ...
                api::client::workspaces::files::add(&remote_repo, &workspace_id, &directory, files_to_add).await?;
                ...
                api::client::workspaces::files::rm_files(&remote_repo, &workspace_id, paths_to_remove).await?;

                // Check status: only one file should be staged for removal
                let status_opts = StagedDataOpts::from_paths_remote_mode(&[images_dir.clone()]);
                let status = repositories::remote_mode::status(&cloned_repo, &remote_repo, &workspace_id, &directory, &status_opts).await?;
                assert_eq!(status.staged_files.len(), 1);
    ...
    #[tokio::test]
    async fn test_remote_mode_rm_dir_with_slash() -> Result<(), OxenError> {
        test::run_training_data_fully_sync_remote(|_local_repo, remote_repo| async move {
            ...
            test::run_empty_dir_test_async(|dir| async move {
                ...
                let workspace_id = cloned_repo.workspace_name.clone().unwrap();
                let directory = ".".to_string();
                let path_with_slash = PathBuf::from("annotations").join("train").join("");
                ...
                let status_opts = StagedDataOpts::from_paths_remote_mode(&[PathBuf::from("annotations")]);
                let status = repositories::remote_mode::status(&cloned_repo, &remote_repo, &workspace_id, &directory, &status_opts).await?;
                
                let files_to_be_removed = [PathBuf::from("annotations/train/one_shot.csv"),
                ...
                assert_eq!(status.staged_files.len(), files_to_be_removed.len());
