...
use crate::view::{Pagination, JsonDataFrame, JsonDataFrameView};
...
pub fn compare_files( // TODONOW split this function up!!!
    ...
    if !schema_1.has_field_names(&required_fields) {
        return Err(OxenError::InvalidSchema(
            Box::new(schema_1)
        ))
    }
    ...
    let targets = targets.iter().map(|target| target.as_str()).collect::<Vec<&str>>();


    let compare = compute_row_comparison(&df_1, &df_2, keys, targets, opts)?;
...
fn compute_row_comparison(
    ...
    println!("columns of df after join are {:?}", joined_df.get_column_names());
    let df1_unique = joined_df.filter(
        &joined_df.column(format!("{}.right", targets[0]).as_str())?.is_null()
    )?;
    let df2_unique = joined_df.filter(
        &joined_df.column(format!("{}.left", targets[0]).as_str())?.is_null()
    )?;


    // Collect the boolean conditions into a vector
    let different_conditions = targets.iter()
        .map(|target| {
    ...
    let different_mask = different_conditions
        .into_iter()
        .reduce(|acc, mask| acc | mask).unwrap(); // TODONOW 

    // Use the mask to filter the dataframe
    let different_targets = joined_df.filter(&different_mask)?;

    let same_conditions = targets.iter()
        .map(|target| {
            let left = format!("{}.left", target);
            let right = format!("{}.right", target);
            let left_col = joined_df.column(&left)?;
    ...
    let tabular_compare = TabularCompare {
        summary,
        schema_left: Some(Schema::from_polars(&df_1.schema())),
        schema_right: Some(Schema::from_polars(&df_2.schema())),
        keys: keys.iter().map(|key| key.to_string()).collect::<Vec<String>>(),
        targets: targets.iter().map(|target| target.to_string()).collect::<Vec<String>>(),
