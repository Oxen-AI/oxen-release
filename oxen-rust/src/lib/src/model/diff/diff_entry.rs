...
impl DiffEntry {
    ...
    fn r_compute_diff_all_files(
        ...
        let commit_entry_reader = CommitEntryReader::new_from_commit_id(repo, base_commit_id, &object_reader)?;
        let mut dirs = commit_entry_reader.list_dir_children(&path)?;

        let commit_entry_reader = CommitEntryReader::new_from_commit_id(repo, head_commit_id, &object_reader)?;
        ...
        for dir in dirs {
            let base_dir_reader = CommitDirEntryReader::new(repo, base_commit_id, &dir, &object_reader)?;
            let head_dir_reader = CommitDirEntryReader::new(repo, head_commit_id, &dir, &object_reader)?;
    ...
    fn r_compute_removed_files(
        ...
        let commit_entry_reader = CommitEntryReader::new_from_commit_id(repo, commit_id, &object_reader)?;
        let mut dirs = commit_entry_reader.list_dir_children(&path)?;
        dirs.push(path);
        ...
        let mut num_removed = 0;
        for dir in dirs {
            let dir_reader = CommitDirEntryReader::new(repo, commit_id, &dir, &object_reader)?;
