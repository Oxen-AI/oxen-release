...
pub fn get_chunker(algorithm: &Algorithm, chunk_size: usize) -> FrameworkResult<Box<dyn Chunker>> {
    ...
    chunkers
        .remove(chunker_name)
        .ok_or_else(|| FrameworkError::ChunkerNotFound {
            name: chunker_name.to_string(),
        })
}
...
impl Algorithm {
    ...
    fn create_chunker(&self, chunk_size: usize) -> Box<dyn Chunker> {
        match self {
            Algorithm::FixedSize => {
                let chunker =
                    FixedSizeChunker::new(chunk_size).expect("Failed to create FixedSizeChunker");
                Box::new(chunker)
            }
            Algorithm::Copier => {
                let mover = Copier {};
            ...
            Algorithm::FixedSizeMultiThreaded => {
                let chunker = FixedSizeMultiChunker::new(chunk_size, 16)
                    .expect("Failed to create FixedSizeMultiChunker");
                Box::new(chunker)
            }
            Algorithm::FastCDC => {
                let chunker =
                    FastCDChunker::new(chunk_size, 16).expect("Failed to create FastCDChunker");
                Box::new(chunker)
            }
        }
    }
